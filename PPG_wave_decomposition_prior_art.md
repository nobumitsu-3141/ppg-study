...
## 12. 原著入手後の訂正（2026-08-30）

Couceiro 2015 と Epstein 2014 の全文を入手し、本稿の記載を確認した。**二点の訂正**がある。

### 12.1 Couceiro 2015 ― 低相関だったのは RI ではなく R1_2

本稿 §3 で「振幅比はすべての参照値と低相関」と書いたが、これは**分解由来の振幅比 R1_2**に
ついての記述であり、**従来法の反射指標 RI は良好であった**。原著の表5・表6より（全体／ティルト陽性群）:

| 指標 | SBP | MBP | TPRI | PP |
|---|---|---|---|---|
| SI | 0.41 / 0.57 | 0.41 / 0.57 | 0.41 / 0.51 | 0.33 / 0.41 |
| T1_d | 0.39 / 0.53 | 0.38 / 0.53 | 0.37 / 0.46 | 0.34 / 0.39 |
| T1_2 | 0.43 / 0.49 | 0.40 / 0.43 | 0.45 / 0.46 | 0.41 / 0.47 |
| **RI（従来法）** | **0.40 / 0.52** | **0.40 / 0.52** | **0.43 / 0.50** | 0.37 / 0.44 |
| R1_d | 0.36 / 0.49 | 0.37 / 0.44 | 0.42 / 0.45 | 0.33 / 0.40 |
| **R1_2（分解由来）** | **0.21 / 0.23** | 0.22 / 0.24 | **0.26 / 0.27** | 0.25 / 0.26 |

正しい読みは「振幅側が駄目」ではなく **「前進波と反射波に分解して振幅比をとる操作そのものが弱い」**。
本プロジェクトの RI の定義はまさに R1_2 に相当し、VitalDB で陽性対照を通らなかったことは
この先行知見と一致する。

**参照値の由来**: 血圧・TPRI は **Task Force Monitor**（指カフ容積補償法による非侵襲血圧＋
**インピーダンス心拍出量計**による心拍出量）。動脈圧波形解析ではない。いずれにせよ実測値ではなく、
二つの非侵襲推定値から導かれた量である。

**対象と設計**: 原因不明の失神で診断的ヘッドアップティルト試験を予定された55例のうち43例
（ティルト陽性21・陰性22）。起立位相で評価。相関は症例ごとに求めて平均。
**ティルト陽性群でのみ相関が0.4台から0.5台へ上昇**しており、循環変動の大きさが検出可能性を
左右することを示す。

**重要**: 心電図・光電容積脈波は **Philips MP50（臨床ベッドサイドモニタ）**から取得されている。
研究用の光学記録ではない。すなわち **分解由来の振幅比が失敗した研究は本プロジェクトを含めて2つあり、
いずれもモニタ波形を用いている**。処理前の生波形で同じ指標が得られるかは未検証である。

### 12.2 Epstein 2014 ― SI は導管スティフネスの指標である

本稿 §6 で「SI は aPWV の代用ではない」とのみ記したが、原著は**何に応答するか**も示している。
...
| 末梢抵抗 | 2%未満 |
| 末梢コンプライアンス | 2%未満 |
| 大動脈スティフネスのみ | むしろわずかに低下 |

導管スティフネス200%増で脈波の足は60 ms早く到達するのに対し、大動脈のみでは5 msである。
原著の結論は "the SI is primarily affected by changes in the stiffness of all conduit vessels"。

**本プロジェクトへの含意（2点）**

1. **SI を全身血管抵抗の指標として期待するのは筋が悪い。** 末梢抵抗を200%変えても SI は2%しか
   動かない。Couceiro でSIがTPRIと相関したのは、**血圧を介した圧依存性スティフネスの経路**による
   間接的なものである可能性が高い（SI–MBP 0.57 > SI–TPRI 0.51 という順序とも整合）
2. **しかし PWTT を規定するのはまさに導管血管の脈波伝播速度である。** よって
   **SI は aPWV の代用としては不適だが、PWTT を規定する量の指標としては理論的に適切**。
   §9.1 の「第2ピーク＝末梢反射という帰属が支持されない」も、それが PWTT の通過経路そのものの
   性状である以上、この用法とは矛盾しない

3. **術中の陰性結果の説明にもなる。** 導管スティフネスは血管壁の構造的性質であり数時間の手術中に
   大きくは変わらない。実際に動くのは末梢トーヌスと血圧である。**SI は術中に実際に動いている量に
   対して感度を持たない**可能性がある。研究1で平均血圧が ΔPWTT の13.9%を説明した一方、
   血管指標がほとんど説明しなかったことと符合する

4. **研究1c の設計に影響する。** フェニレフリンは主として末梢抵抗に作用するため、ΔT の短縮が
   観察されるとすれば圧依存性を介した間接経路の可能性が高い。**平均血圧で調整した解析を必ず併記**し、
   圧で調整してもなお ΔT が動くかを見る。動けば血管トーヌスそのものへの感度の強い証拠になる

### 12.3 再訂正（2026-08-30・§12.1 の訂正のさらなる訂正）

原著の指標定義（式8〜13）を読み直したところ、§12.1 の記載も不正確であった。**RI は「従来法」ではない。** 6指標すべてが多ガウスモデルの成分から導かれており、違いは**どの成分対を取るか**である。

Couceiro の5ガウスモデルは g1・g2 が主前進波、g3 が収縮後期波（late systolic wave）、g4・g5 が拡張期成分（反射波）である。

| 指標 | 定義 | 成分対 | TPRI との相関（全体／ティルト陽性） |
|---|---|---|---|
| SI | B_d − B_s | 前進波 ↔ 反射波（波形の特徴点） | 0.41 / 0.51 |
| RI | A_d / A_s | 同上 | 0.43 / 0.50 |
| T1_d | B_d − B_1 | 主前進波 P1 ↔ 反射波 Pd | 0.37 / 0.46 |
| **R1_d** | A_d / A_1 | 同上 | **0.42 / 0.45** |
| T1_2 | B_2 − B_1 | 主前進波 P1 ↔ **収縮後期波 P2** | 0.45 / 0.46 |
| **R1_2** | A_2 / A_1 | 同上 | **0.26 / 0.27** |

原著は「T1_d と R1_d は SI・RI と密接に関連する」と明記し、R1_2 については「大動脈のインピーダンス不整合の変化に関連し、augmentation index に近い」としている。

**正しい読み方**

- 「分解由来の振幅比が弱い」は**誤り**。前進波↔反射波の比 R1_d は TPRI と 0.42–0.45 で、時間側に劣らない
- 弱かったのは前進波↔**収縮後期波**の比 R1_2（≈ augmentation index）だけである
- **どの成分対を取るかで結果が正反対になる**

**本プロジェクトへの含意**

本プロジェクトの2カーネル分解では、第2カーネルが収縮後期波と反射波の**両方を吸収する**。したがって本研究の RI は R1_2 と R1_d の中間的な量であり、**Couceiro の結果からはどちらとも言えない**。VitalDB で RI が陽性対照を通らなかったことを「先行知見と一致する」と書いたのは根拠が不十分であった。

**カーネル数の選択が指標の生理学的な意味を変える。** Fleischhauer 2020 が2カーネルを推したのはノイズ・体動耐性と形態保存の観点であり、**成分の生理学的帰属を分離する能力とは別の基準**である。

**具体的な作業項目**: `scripts/11_variants_extract.py` の `fit_beat3`（3カーネル）は現在 dt3・ri3 を**第1↔第2成分**からのみ算出している。これは Couceiro の T1_2・R1_2、すなわち成績の悪かった対に相当する。**第1↔第3成分（R1_d 相当）も算出すべきである。** 変種抽出の走行中はスクリプトを変更しないこと。完走後に追加する。
