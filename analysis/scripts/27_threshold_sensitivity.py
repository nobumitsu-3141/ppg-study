...
`src/pda2.py` の閾値には3つの層がある。
  ...
  (c) **根拠なく私が決めたもの**
                          特徴点の重み 20（Wang は 1〜100 を探索、その中間を取っただけ）、
...
もはや検証ではない（研究0 の第一報で、特徴点法を走らせる前に撤退方針を
書いてしまった失敗と同じ構図になる）。
...
------------
  ...
  **当てはめ層**（当てはめ直しが要る・部分集合で実施）
      特徴点の重み・低域通過・成分間隔の下限・成分を増やすか。これらは当てはめ
...
REFIT = [
    ("凍結値",                    {}),
    ("特徴点の重み 1（重みなし）",   {"w_key": 1.0}),
    ("特徴点の重み 100（Wang 上限）", {"w_key": 100.0}),
    ...
    # 0.9T に切ると特徴点の窓（0.65T・0.85T）と幅の尺度も 0.9 倍になる（T が短くなるため）。
...
def selftest() -> int:
    ...
    # ここがずれていれば、B 層の処理系（_one）が 26番の処理系（indices_for_subject）と別の前処理をしている
    if e2e.exists() and same:
        with tempfile.TemporaryDirectory() as td:
            ...
            if fz is not None:
                ...
                rep("B 層の「凍結値」が 26番の同じ被験者と採否・ΔT で一致（処理系が同一）",
