...
--------------
  ...
  T12 保護されていなかった修正  独立監査で「元に戻しても落ちる検査が無い」と判った修正に、
                       単体の検査を付ける（曖昧判定の配線・重みの近傍・退化入力・no_se・
                       幅の尺度化・低域通過・境界張り付き・fs と t の入口検証・貯留槽の境目・
                       自由度・張り付いた時刻の SE）
  T13 早期振幅比        Hellqvist の p1・b・Am_b/Am_p1 が定義どおりか（26番の第 4 の腕）
...
def selftest(quick: bool = False) -> int:
    ...
    # 不変条件は「**採用するなら ΔT 誤差 SE_DT_MAX_MS（20 ms）未満**」。不採用であることは要求しない
