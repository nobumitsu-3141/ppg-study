...
A群（EV1000・最大204例）
    ...
    「RI は FloTrac の CO 推定を逆に追うか」という別の問いに変わる。**陽性でも証明にならない。**
    除外の判断に使う。ここで SVR 方向にまったく動かなければ話は終わる。
...
B群（Vigilance＝肺動脈カテーテル・5例）
    ...
    **n=5 なので記述のみ。判定に使わない。**大学のガンツ症例で何が見えるかを先に同じ手順で確かめるものである。
...
def run(out_path: str | None) -> int:
  ...
  逆に追うか」という別の問いに変わる。**陽性でも RI が SVR を測る証拠にはならない。**
  ここで SVR 方向にまったく動かない場合にのみ、除外の判断として意味を持つ。""")
  ...
  この群の意義は、**大学のガンツ症例で何が見えるかを先に同じ手順で確かめること**である。""")
    else:
