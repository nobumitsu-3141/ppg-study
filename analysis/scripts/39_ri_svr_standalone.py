#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""【探索・凍結後・単体】RI 系の指標は血管抵抗を追うか、それとも血圧を追うか（別マシン用・自己完結）。

**統計解析計画の凍結（2026-08-28）より後に設計した探索的解析である。**確認的解析ではなく、
結果は仮説の生成にとどまる。37番・38番と同じ問いを、**過去の抽出結果に一切依存せず**に
最初から実行する。
...
そこでの相関は「指標は FloTrac の CO 推定を逆に追うか」という別の問いに変わる。**陽性は証明にならない。**
SVR 方向にまったく動かない場合にのみ除外の判断として意味を持つ。判定（roadmap §9・§12）は動かさない。
...
def dose() -> int:
  ...
  この列が実質的な上限にあたる。指標の ρ はこれと比べて読むこと。""")
    out.append("=" * 92)
    text = "\n".join(out)
    print(text)
...
def stats() -> int:
  ...
  「指標は FloTrac の CO 推定を逆に追うか」という別の問いに変わる。**陽性は証明にならない。**
