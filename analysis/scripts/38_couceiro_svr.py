...
def dose(group: str) -> int:
    ...
    for cid in pick_cases(group, None):
        ...
        if m is not None and len(m):
            frames.append(m)
    out = ["=" * 92,
           f"【診断】SVR の振れ幅で絞ると相関は上がるか（38番 --dose・{group} 群）",
           f"結合できた症例 {len(frames)}。**指標ごとの例数は下の表を見ること**"
           f"（有効ウィンドウが {M37.MIN_WIN} 未満の症例は落ちる）。",
           "追記57 と同じ問いを、37番・38番の全症例で見る。閾値の列は 39番と共有しており、"
           "実行前に固定してある。",
           "=" * 92]
    if not frames:
