...
したがって I4 の門番は「領域内の歪みガウス経路の誤採用率が 10% 以下」（7 巡目の実測 5% の 2 倍。
...
------
    ...
    seed 0・n 150 なら同じ版の pda2 で同じ結果を決定的に再現する（11 巡目の版では領域内の誤採用が
    歪みガウス 3/48・ガンマ 3/68。7 巡目の版では 3/55・7/75。別の環境では規準の際の拍が 1〜2 拍ずれうる）。
...
I4_MAX_RATE_GAMMA = 0.20  # ガンマ経路の退行門番（7 巡目の実測 9% の 2 倍。合成波では領域内でも誤採用があり正しさの保証ではない）
# RI も主要な問い（× 末梢血管抵抗）なのに、これまで精度の門番が ΔT にしか無かった（11 巡目 F3）。
# |RI − 真値| ≥ 0.15 を「外れ」とする（RI の範囲 0.1〜0.9 のおよそ 1/5）。門番は退行の監視で、
# 領域内・歪みガウスの 7 巡目の実測 21.8% の 2 倍を上限にする。正しさの保証ではない
RI_ERR_MAX = 0.15
RI_MAX_RATE = 0.45
...
def main() -> None:
    ...
    print(f"\n  I4 の門番（領域内・歪みガウス）: 誤採用 {n_bad}/{len(sk)} = {rate:.1%}"
          f"（上限 {I4_MAX_RATE:.0%}。7 巡目の実測 5% の 2 倍で、超えたら退行）→ {verdict}")
    ...
    print(f"  RI の門番（領域内・歪みガウス）: |RI誤差| ≥ {RI_ERR_MAX} が {ri_bad}/{len(ri_sk)} "
          f"= {ri_rate:.1%}（上限 {RI_MAX_RATE:.0%}。7 巡目の実測 21.8% の 2 倍）→ "
    ...
    print(f"  I4 の門番（領域内・ガンマ）: 誤採用 {n_bad_g}/{len(gm)} = {rate_g:.1%}"
          f"（上限 {I4_MAX_RATE_GAMMA:.0%}。7 巡目の実測 9% の 2 倍。退行の監視であって正しさの保証ではない）→ {verdict_g}")
