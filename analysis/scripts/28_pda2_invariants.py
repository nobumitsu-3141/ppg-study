...
I8 の採否の反転は不変条件違反とはせず、件数を報告する（規準の際にある拍は数値計算の
細部で反転しうる。多ければ規準が標本化に敏感だという情報になる）。
...
def draw(rng, k: int) -> dict:
    ...
    return dict(hr=hr, notch=notch, dt_true=dt, ri_true=float(rng.uniform(0.10, 0.90)),
    ...
    for route in pda2.ROUTES:
        ...
        if r.get("ok"):
            if not (abs(err) < pda2.SE_DT_MAX_MS):
                viol.append(("I4", tag, f"採用なのに ΔT 誤差 {err:+.1f} ms（SE {r['dt_se_ms']:.1f}・広がり {r['dt_spread_ms']:.1f}）"))
        ...
        if "fs" in extra:
            for fs_new in (250.0, 1000.0):
                ...
                if r.get("ok") and rn.get("ok"):
                    if abs(rn["dt_ms"] - r["dt_ms"]) >= 2.0:
                        viol.append(("I8", tag, f"{fs_new:.0f} Hz で ΔT が {rn['dt_ms'] - r['dt_ms']:+.1f} ms 動く"))
...
def main() -> None:
    ...
    out = Path(args.out) if args.out else ROOT / "data" / "pwdb" / f"_invariants_seed{args.seed}.csv"
    ...
    for code in ("I1", "I2", "I3", "I4", "I5", "I6", "I7", "I8", "I9"):
        print(f"{code:<6}{sum(1 for v in viol if v[0] == code):>6}")
    for v in viol:
        print(f"  [{v[0]}] {v[1]}: {v[2]}")
    # 採用の誤差の分布（採用された拍で）
    for route in pda2.ROUTES:
    ...
    strict = [v for v in viol]
    print("\n" + ("ALL PASS" if not strict else f"違反 {len(strict)} 件"))
