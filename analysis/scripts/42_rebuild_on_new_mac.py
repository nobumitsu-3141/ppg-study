...
再構築は 1 台目とビット単位では一致しない。2026-09-12 の突き合わせでは 862 例中 9 例が
±1 ウィンドウ違い（差し引き +1、161,737 対 161,738）、報告値は 3 桁では動かなかった
（lab_log 追記114）。品質規準の境界にあるウィンドウの採否が機械で入れ替わると読んでいる。
再構築した機械の値を確定値に「直さない」こと。確定値は主解析を回した機械のものである。
"""
