...
## 実データでの状況（caseid=1・2026-08-28）

R波検出と拍の切り出しの修正後:

| 項目 | 値 | 評価 |
|---|---|---|
| R波検出 | 548個 / HR 112拍/分 | 修正前は245個（42%）→ **解消** |
| 拍数比（脈波/R波） | 1.00 | 健全 |
| 相対ノイズ | 0.0061 → アンサンブル5拍 | 目標0.003を満たす |
| PWTT | 中央値 128 ms (IQR 96–152) | 妥当 |
| ΔT（平均波形） | **103–105 ms**（切り出し窓によらず安定） | 生理的 |
| RI（平均波形） | **0.47–0.54** | 生理的 |

**この症例の特徴**: HR 112 の高心拍で、1拍内に切痕も肩も無く単調に減衰する
（反射波が収縮期に完全に埋もれている）。R波から+538ms 以降の山は反射波ではなく次の心拍。
それでも平均波形からは ΔT・RI を安定して取り出せており、
「切痕が見えなくても成分波に分けられる」という PDA 採用の根拠が実データで確認された。

ただし個々の5拍アンサンブルでは RI が 0.39〜0.91 とばらつき、
収束検算を通過してしまう外れ値も出る（既知の盲点）。
本解析は60秒ウィンドウ内の全区間（HR112なら約22区間）の**中央値**を採るため、
この程度のばらつきは吸収される見込み。パイロットで棄却率とともに確認する。
