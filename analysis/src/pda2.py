...
(2) 特徴点の位置が正しいかを検算していなかった（Wang 2013）
...
ERRX_MS = 6.0      # 特徴点の時間位置の絶対誤差の総和 [ms]
ERRY = 0.01        # 特徴点の振幅の絶対誤差の総和
NRMSE_MAX = 0.02   # 重み付き正規化二乗平均平方根誤差
W_KEY = 20.0       # 特徴点に置く重み（Wang は 1〜100 を探索。既定は中間）
...
def _refine(t: np.ndarray, y: np.ndarray, i: int):
    ...
    500 Hz では標本間隔が 2 ms あり、補間しないと特徴点1つあたり最大 2 ms の
    離散化誤差が乗る。特徴点3つで 6 ms となり、Wang の閾値と同じ大きさになってしまう。
    """
    if i <= 0 or i >= len(y) - 1:
...
# 特徴点を探す窓（いずれも**拍の先頭からの割合**）。駆出時間は心拍 50〜120 で 0.30〜0.55T
...
def find_landmarks(t: np.ndarray, y: np.ndarray, force_proxy: bool = False,
    ...
    手順
    ----
    1. 波形そのものの極小（切痕）→ 極大（拡張期ピーク）。Wang 2013 の特徴点そのもの。
    ...
    3. 以前あった「立ち上がり側で2次微分の谷を探して型4とする」手順は削除した。
       ...
       dia_t < sys_t という無意味な特徴点を作っていた。

    force_proxy=True は、データ側が代用点（型3）だったときに**模型側も同じ方法で**
    特徴点を取るための指定。allow_proxy=False は逆に、データ側が極値（型1）だったときに
    模型側にも極値を要求する指定（極値が無ければ特徴点なし＝不合格に倒す）。
    どちらも、定義の違う特徴点同士で Errx を取らないためにある。
    ...
    prom は特徴点の顕著さ。型1 では拡張期ピーク−切痕の高低差（閾値 EXTREMA_MIN_PROM）、
...
def early_features(t: np.ndarray, y: np.ndarray) -> dict:
    ...
    **我々の ΔT はまさに「S と D のピーク間の時間」である。** だから同じ条件で比べる。
...
def _weights(t: np.ndarray, lm: dict, w_key: float = W_KEY, halfwidth_s: float = 0.006):
    """特徴点とその近傍（±6 ms）に重みを置く（Wang 2013 の WLS）。
...
def _wave_starts(t, y, lm, n_waves: int):
    """汎用初期値を主、特徴点由来を副とする（Fleischhauer 2020）。
...
def acceptance(t, y, yhat, lm, w=None, nrmse_max: float = NRMSE_MAX,
    ...
    **当てはまりの良さではなく、特徴点が正しい位置に来たかを見る。**
    ...
    # データ側と同じ種類の特徴点を模型側に要求する（定義の違う特徴点を比べない）。
    # データが極値なら模型も極値（無ければ特徴点なし→不合格）、データが代用点なら模型も代用点
    src = lm.get("source")
    ...
    for kt, kv in (("sys_t", "sys_v"), ("notch_t", "notch_v"), ("dia_t", "dia_v")):
        ...
        if np.isfinite(a) and np.isfinite(b):
            ...
            # 模型側で特徴点が消えたら不合格に倒す。閾値を緩めて走らせる感度解析では
...
def assign_roles(peaks: list, lm: dict, t=None) -> dict:
    ...
    **当てはめに決めさせない。** 規則を先に決め、制約で順序を固定したうえで、
    特徴点に最も近い成分を反射波とする。決められなければその旨を返す。
    ...
    if np.isfinite(dia) and len(cand) > 1:
        # 拡張期の特徴点に最も近い成分を反射波とする。**どれだけ僅差だったかを残す。**
        # 2つの候補が拡張期の特徴点からほぼ等距離なら、どちらを選ぶかは実質くじ引きであり、
...
def decompose(t, y, fs: float, route: str = "skew",
    ...
    accept_proxy       型3（特徴点が代用点）の拍を採用に含めるか。**既定は含めない**（下記）
    ...
    # 特徴点が 2 つ揃わない拍（型4〜5）は、成分を増やしても Errx が計算できないので増やさない。
    # 4,374 名の 1 割がこれなら、無駄な当てはめを 1 割分省ける
    # 説明どおり**データ側**の型で決める。以前は模型側と一致した特徴点の数だったので、
    # データに切痕があっても最初の当てはめが滑らかすぎて特徴点を持たない拍は、増やす機会を
    ...
    if acc["n_landmark_matched"] < 2 and lm["klass"] >= 4:
        reason = "no_landmarks"                 # データ側に特徴点が無い（型4〜5）
    elif not proxy_ok:
