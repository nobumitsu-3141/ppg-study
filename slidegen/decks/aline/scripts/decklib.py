...
# 8-chapter breadcrumb labels (観血的動脈圧測定 / aline deck)
CHAPTERS = ["歴史", "成立", "基礎", "正常", "所見", "異常", "対応", "特殊"]

SW, SH = Cm(33.867), Cm(19.05)   # 16:9
TITLE_LEFT = Cm(2.33)            # テンプレのタイトル枠の左端（ルール: 左マージン>=1.6cm）
CONTENT_RIGHT = Cm(31.9)         # 本文ゾーンの右端の目安（右マージン約2cm）
...
# パンくずがある面: 実機Meiryo(LibreOffice置換のHiraginoより横幅が大きい)でもパンくずに
# 被らないよう、タイトル枠の右端を18.8cm以下に狭める（TITLE_LEFT=2.33cmなので幅は約16.47cm）。
TITLE_RIGHT_BC = Cm(18.8)
def _set_title(slide, text, breadcrumb=False):
    # Fixed 44pt (rule). When a breadcrumb sits top-right (x>=19.25cm), narrow the title
    # placeholder so its right edge stays at/under TITLE_RIGHT_BC (18.8cm) — clear of the
    # pennants even under the wider real-PowerPoint Meiryo metrics — and turn word_wrap ON
    # as a safety net: if a title is still too long it wraps inside that safe box instead of
    # running under/behind the breadcrumb. A too-long title is still a CONTENT error to
    # shorten (never shrink the font, never let it wrap in practice): warn so it can be fixed.
    for ph in slide.placeholders:
        if ph.placeholder_format.idx == 0:
            ...
            ph.top = Cm(1.0); ph.height = Cm(2.62); ph.left = TITLE_LEFT
            width = (TITLE_RIGHT_BC - TITLE_LEFT) if breadcrumb else Cm(29.0)
            ph.width = width
            ph.text_frame.word_wrap = bool(breadcrumb)
            ph.text_frame.vertical_anchor = MSO_ANCHOR.BOTTOM
            ph.text = text
            for p in ph.text_frame.paragraphs:
                for r in p.runs:
                    _set_font(r, size=TITLE_PT, name=JP)   # keep layout gold color
            w = _title_units(text) * (TITLE_PT/72*2.54)
            limit = width / 360000   # EMU -> cm
            if w > limit:
                import sys
                print(f"[title-overflow] {w:4.1f}cm>{limit:.2f} : {text}", file=sys.stderr)
            return ph
    return None

def set_notes(slide, text):
    """スピーカーノートにテキストを設定する（オンスライドを簡略化した際に削った原文を
    そのまま格納するため）。text が空/Noneなら何もしない。"""
    if not text:
        return
...
def bullets_figure(prs, title, items, fig, active=None, cite=None, cite_url=None, size=None,
                    fig_w=None, text_w=Cm(13.5)):
    ...
    Bullets are short one-liners (rule: 各行折り返さない・最大3〜4行), so the text column
    is kept narrow (text_w) and the freed width is handed to the figure, separated by a
    >=0.6cm gap (rule: 図ゾーンと箇条書きゾーンを間隔で分離)."""
    s = content_slide(prs, title, active=active, cite=cite, cite_url=cite_url)
    size = size or 22                             # rule: body text >= 22pt
    y0 = Cm(5.35); bottom = Cm(17.7 if cite else 18.2)
    tb, tf = _txbox(s, Cm(2.2), y0, text_w, bottom - y0)   # TOP anchor
    for i, (txt, kw) in enumerate(items):
    ...
    fig_x = Cm(2.2) + text_w + Cm(0.7)             # gap 0.7cm (>=0.6cm rule)
    fig_w = fig_w or (CONTENT_RIGHT - fig_x)
    add_figure(s, fig, x=fig_x, y=Cm(5.4), w=fig_w, h=Cm(12.0 if not cite else 11.3))
    return s
...
def divider(prs, idx, chapter_names, short_list, subs_map):
    """8-chapter section divider: left = all 8 chapter names, ONE line each (active=gold
    bold, others=gray), right = current chapter's numbered sub-sections in gold.
    subs_map[idx] = [(code,label), ...]. chapter_names must be the SHORT (no-subtitle)
    names — same list as the 目次 (menu_slide) — never the long "name：subtitle" form:
    that used to wrap to 2 lines per item and, because the box was MIDDLE-anchored, the
    resulting overflow pushed the first item up past the gold underline (title-overlap
    bug). Left column is now TOP-anchored with word_wrap off, so each line is guaranteed
    single-line and the box's declared top (safely below the ~3.85cm gold underline) is
    always where the first item actually starts."""
    s = content_slide(prs, f"{idx+1}. {short_list[idx]}", active=idx)
    # left: 8 short chapter names, single font size (22pt, meets body-text minimum) so
    # active/inactive differ by color+weight only, not size — keeps height predictable.
    tbL, tfL = _txbox(s, Cm(2.2), Cm(5.4), Cm(15.0), Cm(12.9), anchor=MSO_ANCHOR.TOP, wrap=False)
    for i, name in enumerate(chapter_names):
        ...
        add_para(tfL, [(f"{i+1}. ", dict(size=22, bold=True, color=GOLD if on else LGRAY)),
                 ...
                 first=(i == 0), space_after=16, line=1.15)
...
def photo_placeholder(slide, x, y, w, h, label="ここに機器の写真を貼付", sub=None):
    ...
    add_para(tf, [("［ 写真スペース ］", dict(size=18, bold=True, color=TEALD))],
             align=PP_ALIGN.CENTER, first=True, space_after=6)
    add_para(tf, [(label, dict(size=14, bold=True, color=GRAY))], align=PP_ALIGN.CENTER, space_after=4)
    if sub:
...
def steps_photo(prs, title, steps, photo_label, photo_sub=None, active=None, cite=None,
    ...
    y0 = Cm(5.05)
    if lead:
        tbl, tfl = _txbox(s, Cm(2.0), Cm(4.65), Cm(19.4), Cm(1.0))
        add_para(tfl, [(lead, dict(size=22, bold=True, color=GOLD))], first=True)
        y0 = Cm(5.95)
...
def references(prs, refs, title="参考文献"):
    """refs: list of (text, url_or_None). 2-column, gray, hyperlinked, 10.5pt.
    title: overridable so a long reference list can be paginated across multiple slides
    (e.g. "参考文献 (1/2)") — caller is responsible for chunking `refs` to fit the box."""
