...
公開の可否について本人の許諾があるかを確認し、無ければ**公開を止める**こと。
判断は著者（川副）が行う。手当ての選択肢は次のとおり。

1. `main` から当該ファイルを削除する（履歴には残るが、サイトからは消える）
2. 履歴からも消す（`git filter-repo`。公開後の複製・キャッシュまでは消せない）
3. 内容を一般化して実名・所属・研究の詳細を除いた版に差し替える
