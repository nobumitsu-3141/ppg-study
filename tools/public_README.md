...
`analysis/README.md` に書いてある。

**`--selftest` は 23 本すべてが、データを 1 つも置いていない状態で通る。**2026-09-10 に、
clone した直後と同じ木（`analysis/data/` が空）で 23 本を順に走らせて確かめた。走行中に
`data/` へ書き出されるのは 26番の照合用 CSV と 33番の進捗の 2 ファイルだけで、Pulse Wave
Database も VitalDB も取得しない。`tests/` の 8 本も同じ状態で通る。

ただし**順番には意味がある。**27番は 26番が残す照合用の出力を読み、それが無い場合は
26番との整合の検算を省略したうえで `ALL PASS` と表示する（省略したことは画面に出る）。
27番を単独で走らせた `ALL PASS` は 27番だけの検査である。26番を先に走らせること。
